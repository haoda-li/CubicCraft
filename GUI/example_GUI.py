...
class AppWindow:
    ...
    def __init__(self, width, height):
        ...
        Own settings
        """
        cubic_settings = gui.CollapsableVert("Cubic settings", 0,
                                                gui.Margins(em, 0, 0, 0))
        
        self._step_size = gui.Slider(gui.Slider.INT)
        self._step_size.set_limits(1, 10)
        self._step_size.set_on_value_changed(self._on_step_size)

        grid = gui.VGrid(2, 0.25 * em)
        grid.add_child(gui.Label("Step size"))
        grid.add_child(self._step_size)
        cubic_settings.add_child(grid)

        self._settings_panel.add_child(cubic_settings)
        # ----
    ...
    def _apply_settings(self):
        ...
        self._step_size.double_value = self.settings.step_size
    ...
    def _on_step_size(self, size):
        self.settings.step_size = int(size)
        self._apply_settings()
    ...
    def _on_menu_cubic(self):
        ...
        if judge_temp == False:
            ...
            for i in range(self.settings.step_size):
